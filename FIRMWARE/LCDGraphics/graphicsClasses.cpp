...
writeText(x+10, y+10, 3, this->title, BLACK, this-> background);
...
bool Button::check(PVector a){
return ( (a.x > x) && (a.x < (x+dx)) && (a.y > y) && (a.y < (y+dy)));
}

bool Button::checkWithTitle(PVector a, string title){
...
Keyboard::Keyboard(){
       //int nextLineIndex
       vector< vector<string> > keyTitleLower;//= {{ "1", "2", "3", "4", "5", ":"}, {"6", "7", "8", "9", "0", "â†", "X"}};
       char* firstLine[6];
       char* secondLine[6];
       firstLine[0] = "0";
       firstLine[1] = "1";
      ...
      // keyTitleLower.push_back(secondLine);
       ...
       for(int i = 0; i < 6; i++){
         ...
         n++;
         //b.draw();
         //writeText(b.x, b.y, 4, (char*)b.title.c_str(), BLACK, RED);
...
Keyboard::Keyboard(int i){
      ...
      // keyTitleLower.push_back(secondLine);
       ...
       for(int i = 0; i < 7; i++){
         ...
         Button b( i*60+(55-35), 210, 45, 50, RED, WHITE, firstLine[i]);
         keys[n] = b;
         n++;
         //b.draw();
         //writeText(b.x, b.y, 4, (char*)b.title.c_str(), BLACK, RED);
       ...
       for(int i = 0; i < 6; i++){
           Button b( i*60+(55-35), 270, 45, 50, RED, WHITE, secondLine[i]);
           keys[n] = b;
           n++;
           //keys[6+i]  = Button(i*45+(55-35), 260+55*1+100, 35, 45, 0x8080, 0x0, keyTitleLower[1][i]);
       }
...
void Keyboard::draw(){
    ...
    fillRect(55,200,400,150,BLACK);
    //fillRect(0,0,320,200, BLACK);
...
void Nav_bar::draw(char* day){
   ...
   for(int i = 0; i <7; i++){
     ...
     if (elements[i].title == day){
         writeText(_x, 60, 3,this->elements[i].title, BLACK, YELLOW);
     }
     else{
         writeText(_x, 60, 3,this->elements[i].title, BLACK, WHITE);
     }
     //stroke(bg);
